    ...
    return (
      <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-emerald-50 via-white to-teal-50">
        <div className="text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-800 mb-2">Masjid Tidak Ditemukan</h1>
          <p className="text-slate-500">Link pantau yang Anda buka tidak valid.</p>
        </div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 via-white to-teal-50 p-4 md:p-8">
      <div className="max-w-5xl mx-auto">
        ...
        <div className="text-center mb-8">
          <div className="w-14 h-14 mx-auto rounded-2xl bg-emerald-600 flex items-center justify-center text-white shadow-lg shadow-emerald-600/30 mb-4">
          ...
          <h1 className="text-2xl font-bold text-slate-800">{profile?.nama_masjid || "..."}</h1>
          <p className="text-slate-500 text-sm mt-1">{profile?.alamat_masjid}</p>
          <p className="text-emerald-600 text-xs font-medium mt-2">📊 Pantau Rekapitulasi Zakat Fitrah</p>
        </div>

        {/* Summary Cards */}
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-slate-200 p-4 text-center shadow-sm">
            <p className="text-xs text-slate-400 mb-1">Zakat Beras</p>
            <p className="text-xl font-bold text-emerald-600">{totalZakatBeras} <span className="text-sm font-medium">Kg</span></p>
          </div>
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-slate-200 p-4 text-center shadow-sm">
            <p className="text-xs text-slate-400 mb-1">Zakat Uang</p>
            <p className="text-xl font-bold text-emerald-600">Rp {totalZakatUang.toLocaleString()}</p>
          </div>
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-slate-200 p-4 text-center shadow-sm">
            <p className="text-xs text-slate-400 mb-1">Infaq Beras</p>
            <p className="text-xl font-bold text-teal-600">{totalInfaqBeras} <span className="text-sm font-medium">Kg</span></p>
          </div>
          <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-slate-200 p-4 text-center shadow-sm">
            <p className="text-xs text-slate-400 mb-1">Infaq Uang</p>
            <p className="text-xl font-bold text-teal-600">Rp {totalInfaqUang.toLocaleString()}</p>
          </div>
        </div>

        {/* Table */}
        <div className="bg-white/80 backdrop-blur-xl rounded-2xl border border-slate-200 shadow-sm overflow-hidden relative">
          {loading && (
